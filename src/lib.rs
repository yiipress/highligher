...
fn merge_replacements(html: &str, blocks: &[Block<'_>], replacements: &[Option<String>]) -> String {
...
fn into_raw_c_string(value: String, error_ptr: *mut *const c_char) -> *mut c_char {
    match CString::new(value) {
        Ok(value) => value.into_raw(),
        Err(_) => {
            set_error(
                error_ptr,
                "Highlighted output contains an unsupported NUL byte",
            );
            ptr::null_mut()
        }
    }
...
fn str_to_raw_c_string(value: &str) -> *mut c_char {
    CString::new(value)
        .expect("static error messages do not contain NUL bytes")
        .into_raw()
}
...
fn set_error(error_ptr: *mut *const c_char, message: &str) {
    ...
    let error_msg = str_to_raw_c_string(message);
...
) -> Option<Cow<'static, Theme>> {
    if theme_name_ptr.is_null() {
        set_error(error_ptr, "Highlight theme name pointer is null");
        return None;
    }

    if theme_name_len == 0 {
        set_error(error_ptr, "Highlight theme name must not be empty");
        return None;
    }

    let theme_name_bytes =
        unsafe { slice::from_raw_parts(theme_name_ptr.cast::<u8>(), theme_name_len) };
    if theme_name_bytes.contains(&0) {
        set_error(
            error_ptr,
            "Highlight theme name contains an unsupported NUL byte",
        );
        return None;
    }

    let requested_theme_name = match str::from_utf8(theme_name_bytes) {
        Ok(s) => s,
        Err(e) => {
            set_error(
                error_ptr,
                &format!("Invalid UTF-8 in highlight theme name: {}", e),
            );
            return None;
        }
    };

    if let Some(theme) = THEME_SET.themes.get(requested_theme_name) {
        return Some(Cow::Borrowed(theme));
    }

    if Path::new(requested_theme_name)
        .extension()
        .is_some_and(|extension| extension.eq_ignore_ascii_case("tmTheme"))
    {
        return match ThemeSet::get_theme(requested_theme_name) {
            Ok(theme) => Some(Cow::Owned(theme)),
            Err(e) => {
                set_error(
                    error_ptr,
                    &format!(
                        "Failed to load highlight theme \"{}\": {}",
                        requested_theme_name, e
                    ),
                );
                None
            }
...
/// (or null if successful). The error message must be freed by calling `highlighter_free`.
/// The returned pointer must be freed by calling `highlighter_free`.
#[no_mangle]
pub unsafe extern "C" fn highlighter_highlight(
    ...
    if html_bytes.contains(&0) {
        set_error(error_ptr, "HTML input contains an unsupported NUL byte");
        return ptr::null_mut();
    }
    ...
    let result = if blocks.len() < PARALLEL_BLOCK_THRESHOLD {
        ...
        let replacements: Vec<Option<String>> = blocks
            .par_iter()
            .map(|block| highlight_block(ss, theme.as_ref(), block))
            .collect();
    ...
    into_raw_c_string(result, error_ptr)
}
...
/// The returned pointer must be freed by calling `highlighter_free`.
#[no_mangle]
pub unsafe extern "C" fn highlighter_highlight_code(
    ...
    if code_bytes.contains(&0) {
        set_error(error_ptr, "Code input contains an unsupported NUL byte");
        return ptr::null_mut();
    }
    ...
    if language_bytes.contains(&0) {
        set_error(error_ptr, "Language input contains an unsupported NUL byte");
        return ptr::null_mut();
    }
    ...
    let language = match str::from_utf8(language_bytes) {
        ...
        Err(e) => {
            set_error(
                error_ptr,
                &format!("Invalid UTF-8 in language input: {}", e),
            );
            return ptr::null_mut();
        }
    };
    ...
    let result = match highlight_code(ss, theme.as_ref(), language, code) {
        Some(result) => result,
        None => code.to_owned(),
    };
...
/// Frees a string previously returned by `highlighter_highlight`.
///
/// # Safety
///
/// `ptr` must be a pointer returned by `highlighter_highlight` or
/// `highlighter_highlight_code`, or null.
#[no_mangle]
pub unsafe extern "C" fn highlighter_free(ptr: *mut c_char) {
    if !ptr.is_null() {
        unsafe {
